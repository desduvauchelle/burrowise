...
export function proxy(request: NextRequest) {
	...
	if (pathname.startsWith('/api/')) {
		...
		if (origin) {
			const defaultDesktopOrigins = process.env.NODE_ENV === 'production' ? 'tauri://localhost,http://tauri.localhost,https://tauri.localhost' : 'tauri://localhost,http://tauri.localhost,https://tauri.localhost,http://localhost:4173'
			const desktopOrigins = [...new Set(`${defaultDesktopOrigins},${process.env.DESKTOP_APP_ORIGINS || ''}`.split(',').map((value) => value.trim()).filter(Boolean))]
			const host =
				request.headers.get('x-forwarded-host') ?? request.headers.get('host') ?? ''
			try {
				if (new URL(origin).host !== host && !desktopOrigins.includes(origin)) {
